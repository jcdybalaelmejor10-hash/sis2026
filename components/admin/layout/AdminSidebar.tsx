...
export default function AdminSidebar({ isOpen, toggleSidebar }: SidebarProps) {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      href: '/dashboard/admin/coverage-zones',
    },
    {
      title: 'Recolectores',
      icon: (
    ...
    },
    {
      ...
      href: '/dashboard/admin/company-services',
    },
    {
      title: 'Solicitudes',
      icon: (
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-50 h-screen bg-gradient-to-b from-[#1a1d1f] via-[#2F3437] to-[#1a1d1f] border-r border-[#5C8F2B]/20 transition-transform duration-300 shadow-2xl ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-6 border-b border-[#5C8F2B]/20 bg-gradient-to-r from-[#5C8F2B]/10 to-transparent">
            ...
            <button
              onClick={toggleSidebar}
              className="text-gray-300 hover:text-[#9ACD32] transition-colors"
            >
            ...
            {menuItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.href}
                  onClick={toggleSidebar}
                  className={`group flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 relative overflow-hidden ${
                    isActive
                      ? 'bg-gradient-to-r from-[#5C8F2B] to-[#4A7322] text-white shadow-lg shadow-[#5C8F2B]/30'
                      : 'text-gray-300 hover:bg-[#5C8F2B]/10 hover:text-[#9ACD32]'
                  }`}
                >
                  {isActive && (
                    <div className="absolute inset-0 bg-gradient-to-r from-[#9ACD32]/20 to-transparent animate-pulse"></div>
                  )}
                  <div className={`relative z-10 ${
                    isActive ? 'text-white' : 'text-gray-400 group-hover:text-[#9ACD32]'
                  } transition-colors`}>
                    {item.icon}
                  </div>
                  <span className="font-medium relative z-10">{item.title}</span>
                  {isActive && (
                    <div className="absolute right-0 top-0 bottom-0 w-1 bg-[#9ACD32] rounded-l-full"></div>
                  )}
                </Link>
              );
            })}
          ...
          <div className="p-4 border-t border-[#5C8F2B]/20 bg-gradient-to-r from-[#5C8F2B]/10 to-transparent">
            <div className="flex items-center space-x-3 px-4 py-3 bg-[#2F3437]/50 backdrop-blur-sm rounded-lg border border-[#5C8F2B]/20 hover:border-[#5C8F2B]/40 transition-all group cursor-pointer">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-white truncate group-hover:text-[#9ACD32] transition-colors">Administrador</p>
                <p className="text-xs text-gray-400 truncate">admin@ecoforge.com</p>
              </div>
            </div>
          </div>
        </div>
